...
- **没有 Session 删除控件**：会话可以归档与取消归档，但本界面从不删除会话日志；删除 Workspace 注册记录也不会删除 Session。
