...
将 SOYM wiki 工作区中的待提交改动作为一次 git 提交。请在做出改动的同一会话中运行它：SOYM 操作纪律（铁律9）要求代码/测试/配置改动在会话内提交——auto-commit 只进数据产物，会让源码改动堆积未提交。不传 `paths` 时暂存整个工作树（`git add -A`）；传 `paths` 则只提交子集。没有可暂存内容时报告 committed=false 而非失败。
